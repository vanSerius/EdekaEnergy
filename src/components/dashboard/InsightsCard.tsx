      ...
      <div className="relative flex items-center justify-between">
        <div>
          ...
          <h3 className="mt-1.5 font-display text-2xl font-semibold tracking-tight text-ink">
            Was uns <span className="serif-italic text-edeka-blue">auffällt</span>.
          </h3>
        </div>
      </div>
      ...
      <ul className="relative mt-6 space-y-4">
        {insights.map((i, idx) => {
          ...
          return (
            ...
            >
              <span className={`shrink-0 rounded-2xl p-2 ${colorFor(i.tone)}`}>
                <Icon className="h-4 w-4" strokeWidth={2.2} />
              </span>
              <p className="flex-1 text-sm leading-relaxed text-ink">{i.text}</p>
              <ArrowUpRight
